...
- `results/dataset_id`
  ...
  - `tree_supports/` the **Bootstrap support values** are stored as node labels in the trees found in tree_supports folder. Here you will find one folder with the trees with the <ME|ML> topology and the <ME|ML> support values (${id}_<ME|ML>_tree_<ME|ML>_bs.nwk), the IMD support values (${id}_<ME|ML>_tree_IMD_bs.nwk) and the multistrap support values (${id}_<ME|ML>_tree_multistrap_bs.nwk). Additionally, you will find the IMD folder, with the IMD tree with the IMD bootstrap support values (${id}_IMD_tree_IMD_bs.nwk).
  </details>
